...
const command = function (cmd, user, char1) {
  ...
  // if calculating damage "!damageCalc GUARD DODGE damage"
    ...
    let dmgOut = mitigatedDmgCalc(dmgIn, guard, dodge)
    retVal.msg += "dano **antes** da mitigação: " + dmgIn + "\n"
    retVal.msg += "dano reduzido: " + dmgOut + "\n"
    retVal.msg += "dano tomado: " + (dmgIn - dmgOut) + "\n"
  }
  ...
  // if taking damage "!takedmg (num)"
    ...
    if (!/[0-9]+/.test(cmd))
      ...
      let preMitiDmg = Number(/[0-9]+/.exec(cmd)[0]);                 // damage taken PRE mitigation
      let mitigatedDmg = mitigatedDmgCalc(preMitiDmg, guard, dodge);  // damage mitigated

      // deal damage
      postMitiDmg = preMitiDmg - mitigatedDmg
      char.HP.current -= postMitiDmg;

      // message user in discord
      retVal.msg += "dano **antes** da mitigação: " + preMitiDmg + "\n"
      retVal.msg += "(GUARD: " + guard + ", DODGE: " + dodge + ")\n\n"
      retVal.msg += "dano reduzido: " + mitigatedDmg + "\n"
      retVal.msg += `dano tomado: ${preMitiDmg - mitigatedDmg}\n\n`
 ...
 * @param {number} dodge 
 */
const mitigatedDmgCalc = (dmgIn, guard = 0, dodge = 0) => {
  if (dodge < 0) dodge = 0
  let mitigDmg = dmgIn - (Math.floor(dmgIn * (15 / (dodge + 15))) - guard)
  if (mitigDmg > dmgIn)
    return dmgIn
  else
    return mitigDmg
