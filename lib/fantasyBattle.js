 ...
 * 
 * @param {string}   att        attribute to be used in function call
 * @param {(att: string) => any} ifValid    function to be executed on valid attributes. uses att as argument
 * @param {(att: string) => any} ifInvalid  function to be executed on invalid attributes. uses att as argument
 * 
 * @returns {boolean} returns true if att is a valid attribute, false if not
 */
const useAttribute = (att, ifValid = () => { }, ifInvalid = () => { }) => {
  const attOriginal = att
