...
const command = function (cmd, user, char1) {
  ...
  // if changing a Defense's base value "!set (Defense) x"
    ...
    (def) => {
      ...
      else if (/auto/i.test(cmd.split(' ')[2])) {
  ...
  else if (isInArray(command, CommandWords.set) && isInArray(command, ['size', 'tamanho'])) {
    if (cmd.split(' ').length < 3 || !/[0-9]/.test(cmd)) {
      retVal.msg += "Aprenda a usar o comando. o formato certo é \"!set size x\""
    }
    else {
      const oldSize = char.size.base
      const newSize = Number(/[0-9]+/.exec(cmd)[0])
      char.size.base = newSize < 1 ? 1 : newSize
    ...
    switch (user.FB.conversation.stage) {
      ...
      case 28:
        ...
        // if number entered 
          ...
          retVal.msg = "" + Number(cmd.trim()) + " de Dodge. Última pergunta! Qual o tamanho do personagem? (pergunte pro mestre se não sabe, ou coloque 5)"
          char.Dodge = {
            base: Number(cmd.trim()),
            bonus: 0
          }
...
const getDefense = (def, char) => def === 'guard' ?
  0.75 * char.Agility.base + 0.5 * char.Perception.base + 0.75 * char.Deception.base - 0.5 * char.Might.base - (char.size-5) :
  0.75 * char.Might.base + 0.5 * char.Fortitude.base + 0.5 * char.Protection.base + (char.size-5)
