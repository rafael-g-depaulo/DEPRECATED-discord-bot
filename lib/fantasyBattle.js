...
const _SEND_CARD_ATTACH_ = false   // wether or not !draw sends a picture of the drawn card
/** OPTIONS FOR THE MODULE ************************************************************/
...
const command = function (cmd, user, char1) {
  ...
  // if using a card "!card"
    ...
    if (!char.hasOwnProperty("cards") || Object.keys(char.cards).length === 0) {
      ...
      if (Object.keys(char.cards).length === 1) {
        ...
        user.FB.conversation = user.FB.conversation || {}
  ...
  // if changing a Defense's base value "!set (Defense) x"
    ...
    (def) => {
      ...
      // if auto setting defense
      if (/auto/i.test(cmd.split(' ')[2])) {
        char[def].base = getDefense(def, char)
      ...
      // else if didn't give a number
      else if (cmd.split(' ').length < 3) {
        retVal.msg += "Aprenda a usar o comando. o formato certo é \"!set (dodge|guard) (auto|x)\""
        return
      }

      // getting the new value and setting it in the character
      if (/-/.test(cmd))
  ...
  // if changing the character's size "!set size x"
  else if (isInArray(command, CommandWords.set) && isInArray(cmd.split(' ')[1], ['size', 'tamanho', 'tam'])) {
    if (cmd.split(' ').length < 3 || !/[0-9]/.test(cmd)) {
      retVal.msg += "Aprenda a usar o comando. o formato certo é \"!set size x\""
    }
    else {

      const oldSize = char.base
      const newSize = Number(/[0-9]+/.exec(cmd)[0])
      char.base = newSize < 1 ? 1 : newSize
      
      retVal.msg += `Tamanho de ${char.name} mudado, de ${oldSize} para ${newSize}`
    }
  }
  // if changing the character's size "!set level x"
  else if (isInArray(command, CommandWords.set) && isInArray(cmd.split(' ')[1], CommandWords.level)) {
  ...
  // if changing the ammount of money a character has. "!money (+|-) x (bronze|prata|ouro)"
  else if (isInArray(command, CommandWords.money)) {
    const regex = new RegExp(`(\\+|add|remove|-) *([0-9]+) *(${Object.values(MoneyWords).map(x => x.join('|')).join('|')})`, 'i')
    if (!regex.test(cmd)) {
      const getMoneyStrings = ({gold, silver, copper}) => [
        gold   > 0 ? `${gold} moedas de ouro.` : false,
        silver > 0 ? `${silver} moedas de prata.` : false,
        copper > 0 ? `${copper} moedas de bronze.` : false,
      ]

      const moneyStrs = getMoneyStrings(char.money)
      
      retVal.msg += `${char.name} tem ${moneyStrs.filter(x => x).join(' ')}`
    }
    else {
      ...
      if (kind === 'invalid') {
        ...
        const isAdd = /(\+|add)/i.test(addOrRemove)
        const ammount = isAdd ? Number(ammount_str) : -1 * Number(ammount_str)

        // if the character doesnt have money, give them money
        if (!char.money) char.money = {
          gold: 0,
          silver: 0,
          copper: 0,
        }

        if (isNaN(char.money[kind])) char.money[kind] = 0

        if (!isAdd && char.money[kind] < Math.abs(ammount)) {
          retVal.msg += `${char.name} não tem moedas de ${kind} o suficiente para retirar ${ammount} moedas. ${char.name} atualmente tem ${char.money[kind]} moedas de ${kind}`
        }
        else {
          char.money[kind] += ammount
          retVal.msg += `${isAdd ? 'Adicionado' : 'Removido'} ${Math.abs(ammount)} moedas de ${kind} ${isAdd ? 'para' : 'de'} ${char.name}. (${char.money[kind] - ammount} -> ${char.money[kind]})`
        }
      }
    }
  }
  ...
  // if having a long rest "!longrest"
    ...
    let HPdice = Dice.rollDie(getDice(char[HPattb].base));
    let MPdice = Dice.rollDie(getDice(char[MPattb].base));
    ...
    retVal.msg += "HP recuperado: (" + HPbaseGain + ") + " + HPdice.sum + "\n" +
                  "MP recuperado: (" + MPbaseGain + ") + " + MPdice.sum + "\n\n";
  ...
  // if taking damage "!takedmg (num)"
    ...
    if (!/[0-9]+/.test(cmd)) {
      ...
      if (dmgOut < 0) dmgOut = 0;                   // make sure char cant mitigate more damage then recieved

      // deal damage
      char.HP.current -= dmgOut;
  ...
  // if checking a character's bio "!bio" (THIS SHOULD ALWAYS BE THE LAST COMMAND CHECKED, BECAUSE OF THE "isInArray(command, getCharNames(user))" PART)
    ...
    retVal.msg += "**" + char.name + "**  "
    retVal.msg += "(AC: " + (char.Dodge.base + char.Dodge.bonus + 8) + ")\n\n"
    retVal.msg += "HP:   **" + char.HP.current + " / " + char.HP.max + "**\n"
...
const conversation = function (cmd, user) {
  ...
  if (!user.FB.hasOwnProperty('conversation'))
    return;
  ...
  if (user.FB.conversation.name === "creating character") {
    ...
    switch (user.FB.conversation.stage) {
      ...
      case 2:
        // if yes
        if (isYes(cmd)) {
          retVal.msg = "E qual o level de " + char.name + "?"
          user.FB.conversation.stage++
          break;
        }
        // if no
      ...
      case 3:
        // if NaN entered
        if (!/[0-9]+/.test(cmd)) {
          retVal.msg = "\"" + cmd.trim() + "\" não é um número. Qual o level de " + char.name + "?"
        }
        // if number entered 
        else {
          char.level = Number(/[0-9]+/.exec(cmd)[0])
          retVal.msg += `Então ${char.name} é Level ${char.level}, massa! E quanto ${char.name} tem de Agility?`
          user.FB.conversation.stage++
        }
        break;
      ...
      case 13: case 14: case 15: case 16: case 17: case 18: case 19: case 20: case 21:
        ...
        if      (attID === 4)  { attributeName = "Agility"; nextAtt = "Fortitude"     }
        else if (attID === 5)  { attributeName = "Fortitude"; nextAtt = "Might"       }
        else if (attID === 6)  { attributeName = "Might"; nextAtt = "Learning"        }
        else if (attID === 7)  { attributeName = "Learning"; nextAtt = "Logic"        }
        else if (attID === 8)  { attributeName = "Logic"; nextAtt = "Perception"      }
        else if (attID === 9)  { attributeName = "Perception"; nextAtt = "Will"       }
        else if (attID === 10) { attributeName = "Will"; nextAtt = "Deception"        }
        else if (attID === 11) { attributeName = "Deception"; nextAtt = "Persuasion"  }
        else if (attID === 12) { attributeName = "Persuasion"; nextAtt = "Presence"   }
        else if (attID === 13) { attributeName = "Presence"; nextAtt = "Alteration"   }
        else if (attID === 14) { attributeName = "Alteration"; nextAtt = "Creation"   }
        else if (attID === 15) { attributeName = "Creation"; nextAtt = "Energy"       }
        else if (attID === 16) { attributeName = "Energy"; nextAtt = "Entropy"        }
        else if (attID === 17) { attributeName = "Entropy"; nextAtt = "Influence"     }
        else if (attID === 18) { attributeName = "Influence"; nextAtt = "Movement"    }
        else if (attID === 19) { attributeName = "Movement"; nextAtt = "Prescience"   }
        else if (attID === 20) { attributeName = "Prescience"; nextAtt = "Protection" }
        else if (attID === 21) { attributeName = "Protection"; nextAtt = false        }
      ...
      case 24:
        ...
        if (user.FB.conversation.stage === 23) {
        ...
        useAttribute(cmd,
          (attr) => {
            ...
            if (nxtResource) {
              ...
              retVal.msg += " Última pergunta! Qual o tamanho do personagem? (pergunte pro mestre se não sabe, ou coloque 5)"
            }
            user.FB.conversation.stage++
          },
          (command) => {
      ...
      // recieving character's size
      case 25:
        // if NaN entered
        if (isNaN(Number(cmd.trim()))) {
          retVal.msg = "\"" + cmd.trim() + "\" não é um número. Quanto " + char.name + " tem de tamanho?"
        }
        // if number entered 
        else {
          retVal.msg = "O tamanho de " + char.name + " é " + Number(cmd.trim()) + ". Criação de personagem completa!"
          char.size = Number(cmd.trim())

          // calculating character's HP, MP, Stamina & defenses
          char.HP = {}
          char.MP = {}
          char.Stamina = {}

          char.HP.max = char.HP.current = getResource('HP', char)
          char.MP.max = char.MP.current = getResource('MP', char)
          char.Stamina.max = char.Stamina.current = getResource('Stamina', char)
          char.Guard = { base: 0, bonus: 0 }
          char.Dodge = { base: 0, bonus: 0 }
          char.Guard.base = getDefense('Guard', char)
          char.Dodge.base = getDefense('Dodge', char)
 ...
 * @param {number} dodge 
 */
const damageCalc = (dmgIn, guard = 0, dodge = 0) => Math.ceil(dmgIn * (15 / ( (dodge > 0 ? dodge : 0) + 15)) - guard)
...
const getResource = (res, char) => {
  const stat = attb => char[attb].base
  const highestSpecial = [
    stat('Alteration'), stat('Creation'), stat('Energy'), stat('Entropy'),
    stat('Influence'), stat('Movement'), stat('Prescience'), stat('Protection')
  ].reduce((acc, cur) => acc > cur ? acc : cur)

  return res === 'HP' ?
    8 + 2* stat('Fortitude') + 2* stat('Presence') + 1* stat('Will') + Math.floor(1.5* stat('Might')) + 2* char.level :
  res === 'MP' ?
    8 + Math.ceil(3* stat('Learning') + 2* stat('Logic') + 1.5* stat('Will') + 1.5* highestSpecial + 2* char.level) :
  res === 'Stamina' ?
    8 + 3* stat('Fortitude') + 2* stat('Agility') + 1* stat('Might') + 2* char.level 
  : 0
}

const getDefense = (def, char) => /dodge/i.test(def) ?
  Math.round(0.75 * char.Agility.base + 0.5 * char.Perception.base + 0.75 * char.Deception.base - 0.5 * char.Might.base - (char.size-5)) :
  Math.floor(0.75 * char.Might.base + 0.5 * char.Fortitude.base + 0.5 * char.Protection.base + (char.size-5))
