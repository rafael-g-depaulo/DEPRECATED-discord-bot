...
const command = function (cmd, user, char1) {
  ...
  // if rolling "!attack [attribute] [(dis)advantage+x] [bonus]"
    ...
    useAttribute(attb,
      Attb => makeAttack(Attb),                       // if there is an attribute, use it
      ()   => makeAttack(char[char.attackAttribute])  // if there isn't an attribute, use the character's attack attribute
    )
  }
